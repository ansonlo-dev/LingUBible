    ...
    function hideBadgeElements() {
        ...
        allElements.forEach(element => {
            try {
                if (!element.classList.contains('recaptcha-badge-hidden') && 
                    !isInForm(element) && 
                    isBottomRightFixed(element)) {
                    
                    // 檢查是否可能是 reCAPTCHA badge
                    const hasRecaptchaIndicators = 
                        element.textContent?.toLowerCase().includes('recaptcha') ||
                        element.innerHTML?.toLowerCase().includes('recaptcha') ||
                        element.className?.toLowerCase().includes('recaptcha') ||
                        element.id?.toLowerCase().includes('recaptcha');
                    
                    if (hasRecaptchaIndicators) {
                        if (hideBadgeElement(element)) {
                            hiddenCount++;
                        }
                    }
